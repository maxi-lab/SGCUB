...
def _list_create(request, model, serializer_class):
	if request.method == "GET":
		return Response(serializer_class(model.objects.all(), many=True).data)
...
def _detail(request, model, serializer_class, pk):
...
def cuota_list_create(request):
	return _list_create(request, Cuota, CuotaSerializer)


@extend_schema(tags=["Finanzas/Cuota"], request=CuotaSerializer, responses=CuotaSerializer)
@api_view(["GET", "PUT", "PATCH", "DELETE"])
def cuota_detail(request, pk):
	return _detail(request, Cuota, CuotaSerializer, pk)
...
def cuenta_corriente_list_create(request):
	return _list_create(request, CuentaCorriente, CuentaCorrienteSerializer)


@extend_schema(tags=["Finanzas/CuentaCorriente"], request=CuentaCorrienteSerializer, responses=CuentaCorrienteSerializer)
@api_view(["GET", "PUT", "PATCH", "DELETE"])
def cuenta_corriente_detail(request, pk):
	return _detail(request, CuentaCorriente, CuentaCorrienteSerializer, pk)
